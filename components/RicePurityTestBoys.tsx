...
export const RicePurityTestBoys: React.FC = () => {
  ...
  const getCategoryScores = () => {
    return boysCategories.map(category => {
  ...
  const copyToClipboard = async () => {
    const text = `I just took the Rice Purity Test for Boys 2025 and scored ${score}/50 (${badge.label})! Take it yourself at ${window.location.origin}/boys`;
    try {
  ...
  // Sharing functions
  const getShareText = () => {
    return `I just took the Rice Purity Test for Boys 2025 and scored ${score}/50 (${badge.label})! 🎯 How do you stack up? Take the test!`;
  };
  ...
  const getCurrentUrl = () => {
    ...
    return 'https://yoursite.com/boys';
  };
  ...
  const shareToInstagram = async () => {
    const text = `I just took the Boys' Rice Purity Test 2025 and scored ${score}/50 (${badge.label})! Take it yourself at ${window.location.origin}/boys`;
    try {
      await navigator.clipboard.writeText(text);
  ...
  const downloadAsImage = async () => {
    try {
      ...
      link.download = 'rice-purity-test-boys-results.png';
  ...
  const downloadAsPDF = async () => {
    try {
      ...
      pdf.save('rice-purity-test-boys-results.pdf');
    } catch (error) {
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-indigo-50 to-slate-100">
      ...
      <header className="sticky top-0 z-50 bg-white/90 backdrop-blur-xl border-b border-blue-200 shadow-lg shadow-blue-200/20">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 py-3 sm:py-5">
          ...
          {/* Progress Bar */}
          <div className="mt-3 sm:mt-5 w-full bg-blue-200 rounded-full h-3 sm:h-4 overflow-hidden shadow-inner">
            <div 
              className="h-3 sm:h-4 bg-gradient-to-r from-blue-500 via-indigo-500 to-purple-500 rounded-full transition-all duration-700 ease-out shadow-lg"
      ...
      <main className="max-w-5xl mx-auto px-4 sm:px-6 py-6 sm:py-10 space-y-6 sm:space-y-10">
        {/* Instructions */}
        <div className="p-6 sm:p-8 bg-gradient-to-br from-white via-blue-50 to-indigo-50 rounded-xl sm:rounded-2xl border border-blue-300 shadow-xl shadow-blue-200/40">
          <h2 className="text-lg sm:text-xl font-black text-slate-800 mb-4 sm:mb-6">
            The Rice Purity Test for Boys
          </h2>
          <div className="mb-4 sm:mb-6 text-center">
            <p className="text-base sm:text-lg font-bold text-blue-600 bg-blue-50 px-3 sm:px-4 py-2 rounded-lg inline-block">
              🔥 Questions tailored for guys
            </p>
          </div>
          <div className="text-slate-700 leading-relaxed space-y-3 sm:space-y-4">
            <p className="italic text-lg sm:text-xl font-semibold text-slate-800">
              The Boys' Rice Purity Test is your chance to see how you stack up against your bros.
            </p>
            ...
            <p className="font-bold text-base sm:text-lg">
              Click every item you've done. Be honest with yourself.
            </p>
          </div>
        </div>

        {/* Call to Action */}
        <div className="p-4 sm:p-6 bg-gradient-to-r from-blue-100 via-indigo-100 to-purple-100 rounded-xl sm:rounded-2xl border border-blue-200 text-center">
          <p className="text-base sm:text-lg font-semibold text-slate-700">
            Want to compare? Check out the{' '}
        ...
        <div className="flex justify-center sm:justify-end">
          <button
            ...
            className="w-full sm:w-auto px-4 sm:px-6 py-2 sm:py-3 text-sm font-bold bg-blue-200 hover:bg-blue-300 text-blue-700 rounded-lg sm:rounded-xl transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 hover:shadow-lg transform hover:scale-[1.02]"
          >
            🗑️ Clear All
          </button>
        </div>
        ...
        <form className="space-y-6 sm:space-y-10">
          {boysCategories.map((category, categoryIndex) => (
            <fieldset key={category.name} className="border-2 border-blue-300 rounded-xl sm:rounded-2xl p-6 sm:p-8 bg-white shadow-xl shadow-blue-200/30 hover:shadow-2xl transition-all duration-300">
              <legend className="text-lg sm:text-xl font-black px-4 sm:px-6 py-2 text-white bg-gradient-to-r from-blue-500 to-indigo-500 rounded-lg sm:rounded-xl shadow-lg">
                {category.name}
              </legend>
              <p className="text-sm sm:text-base text-blue-600 mb-6 sm:mb-8 font-semibold">
              ...
              <div className="grid grid-cols-1 gap-2 sm:gap-3">
                {category.questions.map((question) => (
                  <label 
                    key={question.id}
                    className="flex items-start space-x-3 sm:space-x-5 p-4 sm:p-5 rounded-xl sm:rounded-2xl hover:bg-blue-50 cursor-pointer transition-all duration-200 group border border-transparent hover:border-blue-300"
                  >
                    <input
                      type="checkbox"
                      checked={checkedItems.has(question.id)}
                      onChange={() => handleCheckboxChange(question.id)}
                      className="mt-1 w-5 h-5 sm:w-6 sm:h-6 text-blue-600 bg-blue-100 border-2 border-blue-300 rounded-lg focus:ring-blue-500 focus:ring-2 transition-all duration-200 cursor-pointer"
                    />
                    <div className="flex-1 min-w-0">
                      <span className="inline-block text-xs sm:text-sm font-black text-blue-600 bg-blue-50 px-2 sm:px-3 py-1 sm:py-1.5 rounded-lg mr-3 sm:mr-4 flex-shrink-0">
                        {question.id}
                      </span>
        ...
        {isCompleted && (
          <>
            <div className="mt-12 p-10 bg-gradient-to-br from-blue-50 via-indigo-50 to-purple-50 rounded-3xl border-2 border-blue-200 shadow-2xl shadow-blue-200/40 animate-in slide-in-from-bottom-4 duration-500 hover:shadow-[0_25px_50px_-12px_rgba(0,0,0,0.25)] transition-all duration-500 group results-section">
              ...
              <div className="downloadable-content bg-white p-12 rounded-3xl border border-gray-200" style={{ backgroundColor: '#ffffff', padding: '48px', borderRadius: '24px', border: '1px solid #d1d5db' }}>
                <div className="text-center space-y-8" style={{ textAlign: 'center' }}>
                  <h2 className="text-4xl md:text-5xl font-black text-blue-600 mb-4" style={{ fontSize: '48px', fontWeight: '900', color: '#2563eb', marginBottom: '6px' }}>
                  ...
                  <div className={`inline-block px-8 py-4 rounded-2xl text-2xl font-black shadow-xl mb-4 ${badge.color} ${badge.label === 'Irredeemably Corrupt' ? 'irredeemably-corrupt-mobile' : ''} ${badge.label.toLowerCase().includes('corrupt') ? 'corrupt-result-badge' : 'result-badge-mobile'}`} style={{ 
                    fontSize: '24px', 
                    fontWeight: '900',
                    marginBottom: '16px',
                  ...
                  <div className="text-sm text-blue-500 space-y-1" style={{ fontSize: '14px', color: '#3b82f6' }}>
                    <p className="font-bold" style={{ fontWeight: '700' }}>Average college guy score: ~25</p>
                    <p className="font-medium" style={{ fontWeight: '500' }}>Share with your bros to compare scores!</p>
                  </div>
                </div>
              </div>
                      ...
                      <div className="lg:w-80 bg-white/80 backdrop-blur-sm p-6 rounded-3xl shadow-2xl border border-white/60">
                        ...
                        <div className="space-y-4">
                          <button
                            ...
                            className="w-full flex items-center justify-center space-x-3 p-4 bg-gradient-to-r from-blue-400 to-indigo-600 hover:shadow-2xl text-white rounded-2xl transition-all duration-200 hover:scale-[1.02] active:scale-95 focus:outline-none focus:scale-100"
                          >
                            <svg className="w-6 h-6 transition-transform duration-200" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" />
                            </svg>
                            <span className="text-lg font-bold">Download as Image</span>
                          </button>
        ...
        {isCompleted && (
          <div className="mt-8 p-6 bg-gradient-to-r from-blue-50 via-indigo-50 to-purple-50 rounded-2xl border border-blue-200 text-center hover:shadow-xl hover:scale-[1.01] transition-all duration-200 hover:border-blue-300 group">
        ...
        {/* More Tests Section */}
        <div className="mt-16 p-8 bg-gradient-to-r from-blue-100 to-indigo-100 rounded-2xl border border-blue-300 hover:shadow-xl transition-all duration-200 hover:border-blue-400 group">
        ...
        {/* Footer */}
        <footer className="mt-24 pt-10 border-t border-blue-300 text-center text-sm text-blue-500">
          <p className="font-bold">
            © 2025 Rice Purity Test for Boys • The Modern College Quiz
          </p>
          <p className="mt-3 max-w-3xl mx-auto font-medium">
            This version updates the classic test questions for modern guys.
          </p>
        </footer>
      </main>
    </div>
  );
}; RicePurityTestBoys.displayName = 'RicePurityTestBoys';
